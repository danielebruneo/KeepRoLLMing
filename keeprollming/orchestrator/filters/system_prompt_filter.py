...
from ..filter import Filter, FilterConfig, FilterExecutionContext, Request, register_filter
from keeprollming.logger import log
from keeprollming.logging import get_filter_logger


@register_filter("system_prompt")
class SystemPromptFilter(Filter):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        try:
            self._logger = get_filter_logger("system_prompt")
        except Exception:
            self._logger = None
    ...
    async def process_request(
        ...
        if system_idx is None:
            ...
            if self._logger: self._logger.system_prompt_inserted(self._prompt[:80])
            ...
            if self._logger: self._logger.system_prompt_overridden(self._prompt[:80], len(messages[system_idx].get("content", "")))
            ...
            if self._logger: self._logger.system_prompt_prepended(self._prompt[:80], len(existing))
            messages[system_idx] = {
                "role": "system",
