...
@dataclass
class ReasoningLoopStopperConfig(StreamingFilterConfig):
...
@register_filter("reasoning_loop_stopper")
class ReasoningLoopStopperFilter(StreamingFilterBase):
    ...
    def __init__(self, config):
        ...
        try:
            self._logger = get_filter_logger("reasoning_loop_stopper")
        except Exception:
            self._logger = None
    ...
    async def process_stream_chunk(
        ...
        """Process streaming chunk with reasoning loop detection.

        Delegates to base class which handles buffering; _handle_intervention
        will check for loops when buffer flushes.
        """
    ...
    def _should_start_buffering(self, chunk: bytes, context: FilterExecutionContext) -> bool:
        """Detect when to start buffering reasoning_content chunks.

        Only buffer when reasoning_content has actual content (not empty string).
        """
        try:
            ...
            if not isinstance(delta, dict):
                return False
            # Start buffering only when reasoning_content has actual content
            reasoning = delta.get("reasoning_content")
            return bool(reasoning and isinstance(reasoning, str) and reasoning.strip())
        except Exception:
            return False

    def _should_flush_buffer(self, chunk: bytes, context: FilterExecutionContext) -> bool:
        """Detect when to flush accumulated reasoning buffer."""
        try:
            chunk_str = chunk.decode("utf-8")
            if not chunk_str.startswith("data: "):
                return False
            ...
            if not isinstance(delta, dict):
                return False
            # Flush when we see finish_reason or tool_calls (reasoning complete)
            if delta.get("finish_reason") or delta.get("tool_calls"):
                return True
            # Flush when reasoning_content is no longer present
            if "reasoning_content" not in delta and self._buffering:
                return True
        except Exception:
            pass
        return False
    ...
    async def _handle_intervention(self, context: FilterExecutionContext) -> StreamChunkResult:
        """Handle reasoning loop detection during streaming.
        ...
        # Extract reasoning from accumulated buffer
        combined = b"".join(self._stream_buffer)
        current_reasoning = self._extract_reasoning_from_buffer(combined)

        if not current_reasoning:
            # No reasoning found — flush buffer normally
            return StreamChunkResult(emit=list(self._stream_buffer))

        # Get last reasoning from conversation
        last_reasoning = self._get_last_reasoning(conv)

        # Check for loop: same reasoning AND same tool call
        if last_reasoning and current_reasoning.strip() == last_reasoning.strip():
            # Also check if the LAST tool call is the same (different tool call = not a loop)
            last_tc = self._get_last_tool_call(conv)
            current_tc = self._get_current_tool_call(combined)
            if last_tc and current_tc and last_tc != current_tc:
                # Different tool call → not a loop, flush reasoning
                log("INFO", "rls_flush_different_tc",
                    req_id=req_id,
                    last_tc_name=last_tc.get("function", {}).get("name"),
                    current_tc_name=current_tc.get("function", {}).get("name"))
                return StreamChunkResult(emit=list(self._stream_buffer))

            # Same reasoning AND same/missing tool call → loop detected
            log("INFO", "rls_streaming_loop_detected",
                req_id=req_id,
            ...
            if not upstream_model or not upstream_url:
                log("ERROR", "rls_streaming_retry_missing_context", req_id=req_id)
            ...
            intervention_msg = {
                "role": "user",
                "content": self.config.rls_message,
            }
            ...
            retry_decision = RetryDecision(
                ...
                intervention_message=self.config.rls_message,
            )
    ...
    def _extract_reasoning_from_buffer(self, combined: bytes) -> str:
        """Extract reasoning_content from accumulated buffer."""
        reasoning_parts = []
        try:
    ...
    # ── Response phase (non-streaming + streaming post-process) ──────
    ...
    async def _handle_loop_detected(
        ...
        if self._logger:
            try:
                self._logger.reasoning_loop_detected(reasoning=reasoning[:200])
            except Exception:
                pass
        ...
        if self._logger:
            try:
                self._logger.rls_intervention(messages_count=len(augmented))
            except Exception:
                pass
    ...
    async def _rls_retry(
        ...
        try:
            ...
            return self._handle_fallback(response, req_id)
    ...
    def _handle_fallback(self, response: Response, req_id: str) -> Response:
