...
"""
Debug the full execution with logs from test environment
"""
...
def main():
    print("=== Full Debug Execution with Logs ===")
    ...
    from keeprollming.rolling_summary import should_summarise, _estimate_tokens_for_msgs
    ...
    # Get the message content to understand what's being counted 
    char_count = sum(len(msg['content']) for msg in messages if 'content' in msg)
    print(f"Total character count: {char_count}")
    
    # Let's see how _estimate_tokens_for_msgs works
    estimated_tokens = _estimate_tokens_for_msgs(TOK, messages) 
    print(f"Estimated tokens via _estimate_tokens_for_msgs: {estimated_tokens}")
    ...
    # Check how the test configuration works in detail
    print("\n=== Test Configuration Analysis ===")
    
    # From fake backend config:
    # "models": {
    ...
    print("Test setup:")
    print("- main-model context length: 260")
    print("- summary-model context length: 5000") 
    print("- Summary overflow threshold (chars): 2600")
    
    # The test actually sets the model to be 'summary-model' for the second call
    # In that case, it should go through the summary flow
    
    if plan.should:
        print("\nSUMMARY SHOULD BE TRIGGERED!")
        print("This is likely an issue in execution path not making actual calls")
    else:
        print("\nSUMMARY NOT TRIGGERED - This is unexpected based on our calculations")
