...
def resolve_perf_request_file(perf_dir: Path, client_model: str) -> Path:
    ...
    candidates.append(perf_dir / f"{upstream_model}.requests.jsonl")

    for cand in candidates:
        if cand.exists():
            return cand

    # If no exact match, look for any .requests.jsonl file in the directory
    matches = sorted(perf_dir.glob("*.requests.jsonl"))
    if len(matches) > 0:
        return matches[0]
...
@pytest.mark.non_parallelizable  # Uses shared backend_target fixture that can conflict in parallel runs
