...
class TestModelsEndpoint:
    ...
    def test_models_endpoint_returns_routes(self, client):
        ...
        # Check that we have expected route names (config.test.yaml has test-quick-model, test-main-model, etc.)
        model_ids = {m["id"] for m in data["data"]}
        # Verify we have the route names (not upstream model IDs)
        assert "local/quick" in model_ids or "local/main" in model_ids or "passthrough" in model_ids
    ...
    def test_routes_with_summary_use_max_ctx(self, client):
        ...
        for model in data["data"]:
            if model["id"] in ["local/quick", "local/main", "local/deep", "passthrough"]:
    ...
    def test_route_config_field(self, client):
        """Verify /v1/models returns config with resolved route details."""
        response = client.get("/v1/models")
        data = response.json()
        
        for model in data["data"]:
            cfg = model.get("config", {})
            assert "upstream_model" in cfg, f"Missing upstream_model in config for {model['id']}"
            assert "max_tokens" in cfg, f"Missing max_tokens in config for {model['id']}"
