...
class TestFilterE2EIntegration:
    ...
    async def test_filter_chain_created_from_route_config(self):
        ...
        # Create a mock route with filter_chain config
        route = MagicMock()
        route.filter_chain = {
            "order": ["model_nudge"],
            "filters": {
        ...
        # Verify the route has filter_chain attribute
        assert hasattr(route, 'filter_chain')
...
class TestFilterInheritanceWithExtends:
    """Test that filter_chain config is inherited through route extends."""

    def test_filter_chain_inherited_from_parent(self):
        """Test that child routes inherit filter_chain from parent."""
        ...
        parent_route = Route(
            ...
            pattern="parent/*",
            model="test-model",
            filter_chain={  # Parent has filter_chain
                "order": ["model_nudge"],
                "filters": {
        ...
        child_route = Route(
            ...
            filter_chain=None,  # No override - should inherit
        )

        # Verify both have filter_chain attribute
        assert hasattr(parent_route, 'filter_chain')
        assert hasattr(child_route, 'filter_chain')
        assert parent_route.filter_chain is not None


class TestFilterConfigurationLoading:
    """Test that filter_chain config loads correctly from YAML."""

    def test_filter_chain_from_yaml_config(self):
        """Verify filter_chain can be loaded from configuration."""
        import yaml
        from keeprollming.routing import Route
...
"""
        ...
        assert "filter_chain" in route_data
        assert route_data["filter_chain"]["order"] == ["model_nudge"]

    def test_filter_chain_inheritance_yaml(self):
        """Test filter_chain inheritance through extends in YAML."""
        import yaml
        from keeprollming.routing import Route
...
"""
        ...
        assert "filter_chain" in routes["base_route"]
        # Child should inherit filter_chain from parent


class TestFilterLoggingIntegration:
    """Test that filter logging works end-to-end."""

    def test_filter_logs_to_separate_file(self):
        """Verify each filter logs to its own file."""
        import tempfile
        from pathlib import Path
        from keeprollming.logging import FilterLogger, reset_filter_loggers

        with tempfile.TemporaryDirectory() as tmpdir:
            log_dir = Path(tmpdir) / "filters"

            # Create two different filter loggers
            logger1 = FilterLogger("model_nudge", str(log_dir))
            logger2 = FilterLogger("loop_detector", str(log_dir))

            # Write test events
            logger1.nudge_triggered(":$", "Test:", 1, "nudge", 3)
            logger2.loop_detected(2, 5, "hash123")

            # Verify separate log files
            assert (log_dir / "model_nudge.log").exists()
            assert (log_dir / "loop_detector.log").exists()

            # Verify content
            with open(log_dir / "model_nudge.log", "r") as f:
                entry = json.loads(f.readline())
                assert entry["event"] == "nudge_triggered"

            with open(log_dir / "loop_detector.log", "r") as f:
                entry = json.loads(f.readline())
